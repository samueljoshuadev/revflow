...
import { MetaWhatsAppConnect } from "@/components/integrations/meta-whatsapp-connect";
...
export function IntegrationCenter({
  ...
  const [helpProvider, setHelpProvider] = useState<IntegrationProvider | null>(
    null,
  );
  const [configureProvider, setConfigureProvider] =
    useState<IntegrationProvider | null>(null);
  const helpCard = cards.find((card) => card.provider === helpProvider);
  const configureCard = cards.find(
    (card) => card.provider === configureProvider,
  );
        ...
        <div className="mt-6 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
          ...
          <div>
            <p className="text-sm font-semibold">
              Preparação do servidor pendente
            </p>
            <p className="mt-1 text-xs leading-5 text-amber-800">
              O responsável técnico precisa{" "}
              {!migrationReady
                ? "aplicar a migration da Central de Integrações"
                : "ativar o cofre de credenciais"}
              . Depois disso, os clientes farão todo o restante por esta página.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1fr_320px] lg:items-center">
          <div>
            <div className="flex items-center gap-2 text-xs font-medium text-violet-300">
              <ShieldCheck className="size-4" /> Configuração protegida por
              empresa
            </div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-300">
              Vamos fazer isso juntos. Normalmente leva poucos minutos. Use o
              botão de ajuda em qualquer integração para acompanhar cada etapa.
            </p>
          </div>
...
function IntegrationCard({
  ...
  const googleBlocked =
    card.provider === "google_calendar" && !googleOAuthReady;
  const metaBlocked = card.provider === "whatsapp" && !metaWhatsAppReady;
  const webhookUrl = card.connectionId
    ? card.config.webhook_mode === "shared"
    ...
    <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-xs sm:p-6">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-y border-gray-100 py-4">
        ...
        <p className="text-[11px] text-gray-400">
          {card.lastTestedAt
            ? `Testada ${formatDate(card.lastTestedAt)}`
            : "Ainda não testada"}
        </p>
      </div>
      ...
      {card.secretHint && (
        <p className="mt-4 rounded-lg bg-gray-50 px-3 py-2 text-[11px] text-gray-500">
          Credencial salva:{" "}
          <span className="font-medium text-gray-700">{card.secretHint}</span>
        </p>
      )}
      ...
      {card.lastErrorCode && (
        <details className="mt-3 rounded-lg border border-red-100 bg-red-50 px-3 py-2">
          <summary className="cursor-pointer text-xs font-medium text-red-700">
            Deu erro? Ver detalhes
          </summary>
          <p className="mt-2 break-all font-mono text-[10px] text-red-600">
            {card.lastErrorCode}
          </p>
          {card.diagnosticId && (
            <p className="mt-1 text-[10px] text-red-500">
              Diagnóstico: {card.diagnosticId}
            </p>
          )}
        </details>
      )}
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        ) : card.provider === "whatsapp" && metaWhatsAppReady ? (
          <>
            <MetaWhatsAppConnect
              disabled={!canUse}
              reconnect={Boolean(card.secretHint || isConnected)}
            />
            <Button
              type="button"
              variant="secondary"
              ...
              disabled={!canUse}
            >
              <Settings2 className="size-4" /> Configuração manual
            </Button>
          </>
        ) : (
          <Button type="button" onClick={onConfigure} disabled={!canUse}>
            <Settings2 className="size-4" />{" "}
            {card.secretHint ? "Configurar" : "Conectar"}
          </Button>
        )}
        <form action={testIntegration}>
          ...
          <Button
            ...
            disabled={
              !canUse ||
              (!card.secretHint && card.provider !== "openai") ||
              googleBlocked
            }
          >
        ...
        {(card.secretHint || isConnected) && (
          <form
            ...
            onSubmit={(event) => {
              if (
                !window.confirm(
                  `Desconectar ${card.name} e remover a credencial protegida?`,
                )
              )
      ...
      {googleBlocked && (
        <p className="mt-3 text-[11px] text-amber-700">
          O administrador da plataforma ainda precisa cadastrar o aplicativo do
          Google.
        </p>
      )}
      {metaBlocked && (
        <p className="mt-3 text-[11px] leading-5 text-amber-700">
          O administrador da plataforma ainda precisa cadastrar o aplicativo
          Meta e o Embedded Signup. A configuração manual permanece disponível.
        </p>
      )}
    </article>
  );
...
function HelpDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex justify-end bg-gray-950/50"
      role="presentation"
      onMouseDown={onClose}
    >
      <section
      ...
      >
        <header className="flex items-start justify-between border-b border-gray-200 p-5 sm:p-6">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">
              Vamos fazer isso juntos. Normalmente leva poucos minutos.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
          <div
            ...
            aria-label={`Passo ${step + 1} de ${card.help.length}`}
          >
            {card.help.map((_, index) => (
              <span
                ...
                className={cn(
                  ...
                  index <= step ? "bg-brand" : "bg-gray-200",
                )}
              />
            ))}
          </div>
          <div className="mt-8 flex size-12 items-center justify-center rounded-full bg-violet-50 text-lg font-semibold text-brand">
            {step + 1}
          </div>
          ...
          {current.link && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-5 inline-flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
          ...
          <div className="mt-8 rounded-xl border border-blue-100 bg-blue-50 p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-blue-700">
              No fluxo Meta/Facebook, sua senha nunca passa pelo RevFlow. Na
              configuração manual, não envie tokens, chaves ou senhas por
              mensagem.
            </p>
          </div>
          <details className="mt-4 rounded-xl border border-gray-200 p-4">
            ...
            <p className="mt-3 text-xs leading-6 text-gray-600">
              Confira se sua conta administra o Portfólio Empresarial e o
              WhatsApp Business. Se usar o modo manual, revise os valores
              completos. Depois use “Testar conexão”.
            </p>
          </details>
        </div>
        <footer className="flex items-center justify-between border-t border-gray-200 p-5 sm:p-6">
          ...
          {step < card.help.length - 1 ? (
            ...
            <Button onClick={onClose}>
              <Check className="size-4" /> Entendi
            </Button>
          )}
        </footer>
      </section>
      ...
      >
        <header className="flex items-start justify-between border-b border-gray-200 p-5 sm:p-6">
          <div>
            ...
            <p className="mt-2 text-xs leading-5 text-gray-500">
              O valor secreto será criptografado e nunca será mostrado
              novamente.
            </p>
          </div>
          <button
        ...
        <form action={action} className="space-y-4 p-5 sm:p-6">
          ...
          {card.provider === "whatsapp" && (
            <WhatsAppFields appUrl={appUrl} card={card} />
          )}
          <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 text-[11px] text-gray-600">
            <ShieldCheck className="size-4 text-emerald-600" /> O navegador não
            recebe a credencial depois que ela é salva.
          </div>
          <div className="flex justify-end gap-2 border-t border-gray-100 pt-4">
            ...
            <Button>
              <ShieldCheck className="size-4" /> Proteger e salvar
            </Button>
          </div>
        </form>
      </section>
  ...
  return (
    <>
      <SecretField
        ...
        label={
          card.secretHint
            ? "Nova chave da API (substituirá a atual)"
            : "Chave da API"
        }
        placeholder="sk-..."
      />
      <div>
        ...
        <Select
          id="openai-model"
          name="model"
          defaultValue={String(card.config.model ?? "gpt-5.4-nano")}
        >
          <option value="gpt-5.4-nano">GPT-5.4 nano — econômico</option>
          <option value="gpt-5-mini">GPT-5 mini — equilibrado</option>
          <option value="gpt-5">GPT-5 — mais completo</option>
        </Select>
      </div>
      <div>
        <Label htmlFor="monthly-limit">
          Limite de leads analisados por mês
        </Label>
        <Input
          id="monthly-limit"
          name="monthlyLimit"
          ...
          min={1}
          max={100000}
          defaultValue={Number(card.config.monthly_limit ?? 500)}
          required
        />
      </div>
      <label className="flex items-center gap-3 rounded-lg border border-gray-200 p-3 text-sm text-gray-700">
        <input
          name="automaticQualification"
          type="checkbox"
          defaultChecked={card.config.automatic_qualification === true}
          className="size-4 accent-violet-600"
        />{" "}
        Qualificar automaticamente novos leads
      </label>
    </>
  );
}
...
function WhatsAppFields({
  ...
  const webhook = card.connectionId
    ? card.config.webhook_mode === "shared"
      ? `${appUrl}/api/webhooks/whatsapp/meta`
      : `${appUrl}/api/webhooks/whatsapp/${card.connectionId}`
    : "A URL aparecerá depois do primeiro salvamento";
  return (
    <>
      <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800">
        Modo avançado: use somente se sua empresa administra um aplicativo
        próprio na Meta. Para a maioria das empresas, feche esta janela e use
      ...
      <SecretField
        ...
        name="appSecret"
        label="App Secret"
        placeholder="Cole o segredo do aplicativo"
      />
      <SecretField
        id="verifyToken"
        name="verifyToken"
        label="Verify Token"
        placeholder="Crie uma frase longa e difícil"
      />
    ...
    <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <button
          ...
          className="flex h-8 items-center gap-1 rounded-md border border-gray-200 bg-white px-2 text-[10px] font-medium text-gray-600 hover:bg-gray-100"
        >
